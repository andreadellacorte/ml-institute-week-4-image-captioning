...
def main(
    ...
    # Only load up to 15,000 images from the test split
    dataset = load_dataset(hf_dataset, split="test[:15000]")

    logger.info(f"Dataset loaded.")
    logger.info(f"Dataset info: {dataset}")
    
    # Save all unique images and build index
    logger.info("Saving unique images and building index...")
    image_saved = set()
    index_rows = []
    subset = dataset["test"]
    pbar = tqdm(total=len(subset), desc="Saving images and indexing", unit="row")
    for item in subset:
        ...
        if image_id not in image_saved:
            img_path = images_dir / f"{image_id}.png"
            item["image"].save(img_path, format="PNG", compress_level=0)
            image_saved.add(image_id)
        # Add to index
        index_rows.append({
            "image_id": image_id,
    ...
    pbar.close()
    # Save index as CSV
    index_file = output_path / "index.csv"
    with open(index_file, "w", newline="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=["image_id", "question_id", "multiple_choice_answer"])
        writer.writeheader()
        for row in index_rows:
            writer.writerow(row)
    logger.success(f"Saved {len(image_saved)} unique images to {images_dir}")
    logger.success(f"Saved index with {len(index_rows)} rows to {index_file}")
    
    # Check if we're getting the full dataset
    test_sample = dataset["test"][0]
    logger.info(f"First sample image type: {type(test_sample['image'])}")
    logger.info(f"First sample image format: {test_sample['multiple_choice_answer'].format}")
